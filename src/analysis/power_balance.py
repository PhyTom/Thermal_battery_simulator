...
class PowerBalanceAnalyzer:
    ...
    def compute_power_balance(self) -> PowerBalanceResult:
        ...
        # Per ora assumiamo P_output = 0 (stato stazionario senza scarica)
        P_output = 0.0  # TODO: calcolare da BC convettive interne
    ...
    def _compute_surface_loss(self, face: str) -> float:
        ...
        if face == 'x_min':
            ...
            # Base - conduzione verso terreno
            T_surf = mesh.T[:, :, 0]
            T_ground = mesh.bc_T_inf[:, :, 0]
            k_surf = mesh.k[:, :, 0]
            dA = dx * dy
            # Flusso conduttivo approssimato
            dT = T_surf - T_ground
            q = k_surf * dT / (dz/2) * dA
            return np.sum(q)
